...
with tab2:
    ...
    if not df.empty:
        # 1. 統一欄位名稱，確保與您的列印 HTML 標籤一致，避免 KeyError
        # 這裡根據您的實體表格順序定義
        cols_to_check = ["日期", "車號", "車次", "司機", "路線", "噸數", "進廠", "出車", 
                         "大溪倉", "岡山倉", "時效", "特殊", "紅箱", "營收袋", "剩餘", 
                         "污衣", "潔衣", "舊鞋", "咖啡", "退貨通", "異常", "廠退", 
                         "O2O", "預購", "調撥", "文件", "商品", "借貨", "還貨", 
                         "龍罩", "藍罩", "棧板", "空籃", "空龍", "地墊", "B2C"]
        for c in cols_to_check:
            if c not in df.columns: df[c] = ""

        # 2. 顯示編輯器，並確保變數名稱正確賦值以避免 NameError
        df_display = df.copy()
        df_display.insert(0, "🖨️列印", False)
        
        # 這裡必須確保變數名稱是 edited_df
        edited_df = st.data_editor(
            df_display, 
            hide_index=True, 
            use_container_width=True,
            column_config={"🖨️列印": st.column_config.CheckboxColumn("勾選", default=False)}
        )

        selected_data = edited_df[edited_df["🖨️列印"] == True]
        
        if not selected_data.empty:
            if st.button("🖨️ 生成 5 格 A4 格式預覽 (完成後按 Ctrl+P)"):
                # 3. 處理空值與 NaN
                ...
                for p in range(num_pages):
                    ...
                    for i in range(5):
                        idx = p * 5 + i
                        r = records[idx] if idx < len(records) else {c: "" for c in cols_to_check}
                        
                        # 4. 生成單個 5 行表格 HTML (對齊實體表單格式)
                        table_html = f"""<table class="print-table">
<tr class="bg-gray"><td style="width:14%">配送起訖</td><td style="width:10%">車次</td><td style="width:14%">車號</td><td style="width:14%">運務士簽章</td><td style="width:12%">派車噸數</td><td style="width:12%">進廠時間</td><td style="width:12%">出車時間</td></tr>
...
<tr><td>{r.get('O2O','')}</td><td>{r.get('預購','')}</td><td>{r.get('調撥','')}</td><td>{r.get('文件','')}</td><td>{r.get('商品','')}</td><td>{r.get('借貨','')}</td><td>{r.get('還貨','')}</td></tr>
