...
# --- 1. 頁面基本配置 ---
st.set_page_config(page_title="日翊文化點交系統-正式修復版", layout="wide")
...
st.markdown("<div class='report-title'>🚚 日翊文化點交系統 (全欄位排序修正版)</div>", unsafe_allow_html=True)
...
with tab1:
    with st.form("full_delivery_form", clear_on_submit=True):
        ...
        with c2:
            ...
            car_no = st.text_input("車號", key="input_car_no")
        with c3:
        ...
        with r1:
            ...
            dirty_clean = st.number_input("污衣/潔衣", 0)
            shoes = st.number_input("舊鞋救命", 0)
        with r2:
        ...
        with r3:
            ...
            trans_fac = st.number_input("跨廠調撥", 0)
            factory_back = st.number_input("廠退/退貨通", 0)
        with r4:
            abnormal = st.number_input("異常件(板)", 0)
            o2o_val = st.number_input("O2O(台)", 0)
            borrow_return = st.number_input("借/還貨商品", 0)
        ...
        with e3:
            ...
            b2c_books = st.number_input("書籍退/B2C", 0)
        with e4:
            wp_cage = st.number_input("龍車防水罩", 0)
            wp_bw = st.number_input("藍白防水罩", 0)

        submit = st.form_submit_button("✅ 儲存資料")
        if submit:
            new_entry = {
                "日期": report_date.strftime("%Y-%m-%d"), 
                "車號": car_no if car_no else "未填", 
                "車次": trip, "司機": driver_name, "路線": route,
                "大溪倉": f"{dx_item}:{dx_val}", "岡山倉": f"{ok_item}:{ok_val}", 
                "進廠": in_time.strftime("%H:%M"), "出車": out_time.strftime("%H:%M"), 
                "噸數": tonnage, "時效": fast_val, "紅箱": red_box, "棧板": f"{pallet_type}:{pallet_val}"
            }
            save_data(new_entry)
            st.success("資料已成功儲存！")
            st.rerun()
...
with tab2:
    st.subheader("📊 歷史管理與列印 (1.2.3... 排序)")
    df = load_data()
    if not df.empty:
        # 下方選擇器會依照 1, 2, 3... 由上到下顯示
                ...
                for idx in selected_indices:
                    ...
                    <div class='logistics-card'>
                        ...
                        <table class='print-table'>
                            ...
                            <tr><td>時效：{row['時效']}</td><td>紅箱：{row['紅箱']}</td><td>棧板：{row['棧板']}</td><td>點交日期：{row['日期']}</td></tr>
                        </table>
