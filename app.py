...
with st.expander("基本資料錄入", expanded=True):
    ...
    with col3:
        # 更新：大溪倉與岡山倉細分選項
        daxi_option = st.selectbox("大溪倉細項", ["無", "0.2", "1.3"])
        okayama_option = st.selectbox("岡山倉細項", ["無", "5", "6"])
    with col4:
...
with c3:
    ...
    # 棧板：新增黑色、綠色、木頭選項
    st.write("--- 棧板 ---")
    pallet_type = st.selectbox("棧板材質", ["無", "黑色", "綠色", "木頭"])
    pallet_qty = st.number_input("棧板數量", min_value=0, key="pallet_q")
...
st.divider()
if st.button("💾 儲存點交資料"):
    st.success("資料已成功紀錄！請使用 Ctrl+P 列印紙本報表。")
